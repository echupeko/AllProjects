...
let imgBlock = tableGame.querySelectorAll('img');
...
let max = listCard.length;
    let arr = getRandomArray(max, 6);
    let arr2 = arr.slice(0);
    for (let i = 0; i < arr.length; i++){
        imgBlock[i].src = 'resources/' + listCard[arr[i]] + '.png';
    }
    for (let i = 0; i < arr2.length; i++){
            imgBlock[i+6].src = 'resources/' + listCard[arr2[i]] + '.png';
    }
